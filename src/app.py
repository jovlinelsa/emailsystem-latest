...
base_path = "face_database"
os.makedirs(base_path, exist_ok=True)  # Ensure the face database directory exists
...
def load_trained_model():
    ...
    if faces:
        recognizer.train(faces, np.array(labels))
        logger.info("LBPH recognizer trained successfully.")
    else:
...
def register():
    ...
    try:
        ...
        c = conn.cursor()
        hashed_password = hash_password(password)
        c.execute("INSERT INTO users (email, password, face_path) VALUES (?, ?, '')", (email, hashed_password))
        user_id = c.lastrowid
        face_path = os.path.join(base_path, f"{user_id}.jpg")
